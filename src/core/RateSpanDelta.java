 ...
 */
public class RateSpanDelta implements SeekableView {
  ...
  /**
   * Constructs a {@link RateSpanDelta} instance.
   * @param source The iterator to access the underlying data.
   * @param options Options for calculating rates.
   */
  RateSpanDelta(final SeekableView source, final RateOptions options) {
    this.source = source;
...
//      next_data.reset(0, 0);
   ...
   */
  private void populateNextRateForDecCounter() {
   ...
   * Populate the next rate.
   */
  private void populateNextRateForIncCounter() {
    final MutableDataPoint prev_data = new MutableDataPoint();
    if (source.hasNext()) {
      prev_data.reset(next_data);
      next_data.reset(source.next());
